...
public abstract class BaseTemplate implements XLSTemplate {
	...
	public Row getXLSHeaders(Row row, Sheet s, Integer tipoVacante) {
		...
		if("CORP".equals(this.mode)) {
			int p = 0;
			row = this.getFilterCORPHeaders(row,s, p, tipoVacante);
			p = (int) row.getLastCellNum();
			row = this.getXLSCORPHeaders(row,s, p);
				
		}else if("COM".equals(this.mode)) {
			int p = 0;
			row = this.getFilterCOMHeaders(row,s, p);
			p = (int) row.getLastCellNum();
	...
	public Row getXLSRowMapping(Row row, Sheet s, Object vo, Integer tipoVacante) {
		Integer p = 0;
		if("CORP".equals(this.mode)) {
			 row = this.getFilterCORPRows(row, s, (DatosFiltrosVO) vo, p, tipoVacante);
			 p = (int) row.getLastCellNum();
			 row = this.getXLSCORPRowMapping(row,s, vo, p);
		}else if("COM".equals(this.mode)) {
			 row = this.getFilterCOMRows(row,s,  (DatosFiltrosVO) vo, p);
			 p = (int) row.getLastCellNum();
